...
void ScoreMoves(S_Board* pos, Search_data* sd, Search_stack* ss, S_MOVELIST* move_list, const int ttMove, const int threshold) {
...
int NextMove(Movepicker* mp, const bool skipNonGood) {
    ...
    case GEN_MOVES: {
        ...
        ScoreMoves(mp->pos, mp->sd, mp->ss, mp->moveList, mp->ttMove, mp->threshold);
        ++mp->stage;
        [[fallthrough]];
    }
    case PICK_MOVES: {
