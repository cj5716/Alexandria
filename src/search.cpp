...
int Negamax(int alpha, int beta, int depth, bool predictedCutNode, ThreadData* td, SearchStack* ss, Move excludedMove) {
    ...
    while ((move = NextMove(&mp, skipQuiets)) != NOMOVE) {
        ...
        if (bestScore > -MATE_FOUND) {
            ...
            // Quiet history pruning. At low depths, skip quiet moves with very low history as it suggests the move
            // is unlikely to raise alpha.
            if (   depth <= 8
                && isQuiet
                && moveHistory < -4096 * depth + 4096)
                continue;
        }
