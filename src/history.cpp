...
void updateCorrHistScore(const Position *pos, SearchData *sd, const SearchStack* ss, const int depth, const int diff) {
    ...
    updateSingleCorrHistScore(sd->whitePawnCorrHist[pos->side][pos->whitePawnKey % CORRHIST_SIZE], scaledDiff, newWeight);
    updateSingleCorrHistScore(sd->blackPawnCorrHist[pos->side][pos->blackPawnKey % CORRHIST_SIZE], scaledDiff, newWeight);
...
int adjustEvalWithCorrHist(const Position *pos, const SearchData *sd, const SearchStack* ss, const int rawEval) {
    ...
    adjustment += sd->whitePawnCorrHist[pos->side][pos->whitePawnKey % CORRHIST_SIZE];
    adjustment += sd->blackPawnCorrHist[pos->side][pos->blackPawnKey % CORRHIST_SIZE];
...
void CleanHistories(SearchData* sd) {
    ...
    std::memset(sd->whitePawnCorrHist, 0, sizeof(sd->whitePawnCorrHist));
    std::memset(sd->blackPawnCorrHist, 0, sizeof(sd->blackPawnCorrHist));
